...
interface ElementAttributes{
    ...
    Stack: "horizontal" | "vertical" | "none";
    GridX: number,
    GridY: number,
    GridXPadding: number,
    GridYPadding: number,
...
class Layouter{
    ...
    Window_OnResize(width:number, height:number){
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            this.Recurse(element, <ElementCalculations>{
                Width: width,
                Height: height,
                PaddingT: attr.PaddingTop,
                PaddingB: attr.PaddingBottom,
    ...
    Recurse(elm:HTMLElement, parent:ElementCalculations){
        const attr = this.ReadAttributes(elm);

        const d = <ElementCalculations>{
            Height: 0,
            Width: 0,
            PaddingT: attr.PaddingTop,
            PaddingB: attr.PaddingBottom,
        ...
        if(attr.IgnoreChildren == false){
            for (let i = 0; i < children.length; i++) {
                this.Recurse(<HTMLElement>children[i], d);
            }
        }
    }
    ...
    ReadAttributes(elm:HTMLElement) : ElementAttributes{
        ...
        if(isNaN(marginTAttr) == false) marginT = marginTAttr;
        if(isNaN(marginBAttr) == false) marginB = marginBAttr;
        if(isNaN(marginLAttr) == false) marginL = marginLAttr;
        if(isNaN(marginRAttr) == false) marginR = marginRAttr;
        ...
        if(isNaN(paddingTAttr) == false) paddingT = paddingTAttr;
        if(isNaN(paddingBAttr) == false) paddingB = paddingBAttr;
        if(isNaN(paddingLAttr) == false) paddingL = paddingLAttr;
        if(isNaN(paddingRAttr) == false) paddingR = paddingRAttr;
        ...
        var layoutChildren:boolean = false;
        ...
        return {
            HorizontalAlignment: <any>horizontalAlignment,
            VerticalAlignment: <any>verticalAlignment,
            Height: height,
            Width: width,
            MarginTop: marginT,
